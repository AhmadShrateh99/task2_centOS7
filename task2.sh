...
echo '#!/bin/bash
echo "`sar 1 1|grep -vE "Average" |awk'" '"'NR==4  {OFS="\t\t";  print $3,$4,$5,$6,$7,$8,$9}'"'"'`    `date +%H:%M:%S`   `date +"%d-%m-%y"` "' >cpuUtilization.sh
...
<ul>
  ...
  <li><h4><button onclick="document.location='"'"mem.html"'"'">Memory Info</button></h4></li>
  <li><h5><button onclick="document.location='"'"disk.html"'"'">Disk Usage</button></h5></li>
...
echo -e "Filesystem                      M-Size                  M-Used                 M-Available      Use%           Mounted on        Time         Date" >> /var/www/html/disk.html
...
echo "you need to wait untel the avarage will be calculated"
fi ' > /root/task2/avgs.sh
...
echo "* * * * * /root/task2/avgs.sh > /var/www/html/avgs.html 2>&1 " >> cron_bkp
crontab cron_bkp
rm cron_bkp
